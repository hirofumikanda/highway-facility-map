...
- `verify_counts.py` — 上記2ファイルの件数・内訳・minzoom付与・`lane_count`/
  ...
  地点数、`start_point_name`/`end_point_name`が付与された路線地物数、
  `ROUTE_NUMBERS`対応表のエントリ数、`common_name`/`route_number`がそれぞれ
  付与された路線地物数（うち始点・終点接合部/通称名による追加解決分の内訳を
  含む）、`ROUTE_COMMON_NAMES_BY_ENDPOINTS`/`ROUTE_NUMBERS_BY_COMMON_NAME`
  対応表のエントリ数を報告する
...
- tasks.md タスク番号: 2.1, 2.2, 2.3, 2.4, 2.5（GitHub Issue #2） / 2.1, 2.2（GitHub Issue #59） / 1.1, 1.2（GitHub Issue #91） / 2.1, 2.2, 2.3（GitHub Issue #92） / 3.1, 3.2, 3.3（GitHub Issue #93） / 4.1, 4.2, 4.3（GitHub Issue #94） / 5.1, 5.2（GitHub Issue #95）
