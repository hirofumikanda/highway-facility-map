...
- [x] 6.1 `site/main.js`の路線クリックハンドラを、`法定名`・`通称名`（存在時のみ）・`種別`・`車線数`をラベル付きで表示し、続けて既存の`路線番号`（存在時のみ）を表示する形式に変更する

## 7. タイル再生成と動作確認（Issue: #97）

- [x] 7.1 `pipeline/tilegen/build_lines.sh`等でPMTilesを再生成し、`site/tiles/`へ配置する
- [x] 7.2 ブラウザでサイトを開き、始点・終点接合部により追加解決された路線、および従来どおり法定路線名のみで解決される路線の双方でポップアップ表示（法定名・通称名・種別・車線数・路線番号）を確認する
