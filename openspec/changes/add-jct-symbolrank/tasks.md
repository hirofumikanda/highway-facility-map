## 1. パイプライン: symbolrank算出とminzoom導出
...
- [ ] 4.1 `site/style/map-style.js`の`point-labels`レイヤーに`symbol-sort-key`を追加する。ジャンクションは`symbolrank`（`1`〜`3`）をそのまま使用し、一般IC・スマートIC・その他には既存の種別間優先順位（ジャンクション＞一般IC＞スマートIC＞その他）を保つ固定値（例：`4`・`5`・`6`）を割り当てる式にする。
