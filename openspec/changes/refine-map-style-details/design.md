...
`buildRouteNumberBadgeShieldImageData`が生成する32×32画像に対し、水平方向の伸縮可能領域（`stretchX`）を画像中央付近の限られたピクセル範囲（例：中央から左右対称に数ピクセル幅の帯）に限定し、それ以外の領域（上部矩形の左右端、下部の先細り形状全体を含む輪郭）は伸縮させない。垂直方向（`stretchY`）は指定せず、高さは`icon-size`相当の自然な拡縮のみとする（路線番号バッジの高さはテキスト行数に依存せずほぼ一定のため、高さ方向の9-slice制御は不要）。`content`は、既存の矩形部分（上部60%）の全域として定義する。テキスト自体の余白は`icon-text-fit-padding`側で確保されるため、`content`に追加の余白を持たせない。`content`を矩形部分より小さく（余白付きで）定義すると、`icon-text-fit: "both"`によるフィット計算で「`content`のサイズ＝テキスト＋パディングのサイズ」となるようアイコン全体が拡大され、`content`が小さいほどアイコンが不必要に大きく表示されてしまうため（実装時に実機確認で判明、Risks/Trade-offs参照）。これにより、1桁・2桁いずれの路線番号でも、シールド上部の矩形比率・下部の先細り角度が大きく変わらないまま、必要な幅だけが中央帯で伸縮する。
...
- [Trade-off] `content`を矩形部分より小さく（余白付きで）定義すると、`icon-text-fit: "both"`のフィット計算により、`content`のサイズがテキスト＋パディングのサイズと一致するまでアイコン全体が拡大され、結果としてバッジが不必要に大きく表示される（実機確認で判明。当初`content`に矩形部分内側への余白を持たせていたが、シールドバッジが9-slice未適用時より顕著に大きくなったため、余白なし（矩形部分の全域）に修正した）。テキストの余白は`icon-text-fit-padding`側で確保するため、`content`自体には余白を持たせない。
