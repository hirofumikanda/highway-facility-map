...
## 2. 背景レイヤーのスタイル・地図への統合

- [ ] 2.1 `site/style/map-style.js` に `prefectures` ベクトルタイルソース（`pmtiles://.../prefectures.pmtiles`）を追加する
- [ ] 2.2 都道府県境界の塗りレイヤー・境界線レイヤーを追加し、地理院地図の白地図を参考にした配色（塗り `#f5f5f2`、境界線 `#c9c5bb`、ズーム連動の線幅）で描画する。ラベルレイヤーは追加しない
- [ ] 2.3 `layers` 配列内で都道府県境界レイヤーを `lines-casing` より前（下）に配置し、背景として描画されることを確認する

## 3. ナビゲーションコントロール・URLハッシュ同期
